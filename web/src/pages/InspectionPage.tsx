...
export function InspectionPage() {
  ...
  useEffect(() => {
    if (!file) { setCurrentPreview(null); return; }
  ...
  async function search(event?: FormEvent) {
    event?.preventDefault(); setLoading(true); setError(""); setSummaryError(""); setResult(null); setComparison(null); setReport(null); setSummary(null); setSaved(null);
    try {
      if (mode === "text" && !question.trim()) throw new Error("Enter a question before searching.");
      ...
      if (mode === "image" && file) { try { setSummary(await api.summarizeInspectionImage(file)); } catch (reason) { setSummaryError(reason instanceof Error ? reason.message : "Visual summary unavailable."); } }
  ...
  async function save() {
    if (!result) return;
    try {
      const inspectionQuestion = question.trim() || "Photo inspection";
      let record = mode === "image" && file ? await api.createInspectionWithImage("Office inspection", inspectionQuestion, selected, file) : await api.createInspection("Office inspection", inspectionQuestion, selected);
      if (summary) record = await api.saveInspectionSummary(record.id, summary);
      setSaved(record); if (earlier) setComparison(await api.compareInspection(record.id, earlier));
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Could not save inspection."); }
  }
  async function compare() { if (!saved || !earlier) return; try { setComparison(await api.compareInspection(saved.id, earlier)); } catch (reason) { setError(reason instanceof Error ? reason.message : "Could not compare views."); } }
  async function generateReport() { if (!saved || !earlier) return; setReporting(true); setError(""); try { setReport(await api.inspectionReport(saved.id, question.trim() || "Photo inspection", earlier)); } catch (reason) { setError(reason instanceof Error ? reason.message : "Could not generate inspection report."); } finally { setReporting(false); } }
  ...
  return <>
    ...
    <form className={`panel ${styles.searchPanel}`} onSubmit={search}><div className={styles.segments}><button type="button" className={mode === "text" ? styles.active : ""} onClick={() => setMode("text")}>Ask with text</button><button type="button" className={mode === "image" ? styles.active : ""} onClick={() => setMode("image")}>Upload a photo</button></div>{mode === "image" && <label className={styles.upload}>Choose a PNG or JPEG under 10 MB<input type="file" accept="image/png,image/jpeg" onChange={(event) => setFile(event.target.files?.[0] ?? null)} /></label>}<textarea aria-label="Inspection question" placeholder={mode === "image" ? "Optional: what should I check in this photo?" : "What would you like to find or check?"} value={question} onChange={(event) => setQuestion(event.target.value)} /><div className={styles.confirmActions}><button className={styles.primary} type="submit" disabled={loading || (mode === "image" ? !file : !question.trim())}>{loading ? "Finding evidence…" : "Find evidence"}</button><button className={styles.secondary} type="button" onClick={save} disabled={!result}>{earlier ? "Save and compare" : "Save inspection"}</button></div>{error && <p className="error">{error}</p>}</form>
    {currentPreview && <section className={`panel ${styles.currentPhoto}`}><span className="eyebrow">Current view</span><h2>Photo you uploaded</h2><img src={currentPreview} alt="Uploaded current office view" /></section>}
